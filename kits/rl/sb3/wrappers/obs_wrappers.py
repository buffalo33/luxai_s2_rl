...
class SimpleUnitObservationWrapper(gym.ObservationWrapper):
    ...
    def convert_obs(obs: Dict[str, Any], env_cfg: Any) -> Dict[str, npt.NDArray]:
        ...
        for agent in obs.keys():
            ...
            if len(units.keys()) != 0:
                obs_vec = np.zeros((13,len(units.keys())))

            for k_idx,k in enumerate(units.keys()):
                unit = units[k]

                factories = shared_obs["factories"][agent]
                factory_vec = np.zeros(2)
                closest_factory_dist = None
                for k in factories.keys():
                    # here we track a normalized position of the first friendly factory
                    factory = factories[k]
                    factory_dist = np.mean(
                        (np.array(factory["pos"]) - np.array(unit["pos"])) ** 2
                    )
                    if closest_factory_dist == None or closest_factory_dist > factory_dist:
                        closest_factory_dist = factory_dist
                ...
                obs_vec_unit = np.concatenate(
                    [unit_vec, factory_vec - pos, closest_ice_tile - pos], axis=-1
                )

                obs_vec[:,k_idx] = obs_vec_unit
                obs_vec = np.squeeze(obs_vec)
