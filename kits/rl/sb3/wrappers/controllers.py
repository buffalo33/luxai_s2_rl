...
class SimpleUnitDiscreteController(Controller):
    ...
    def action_to_lux_action(
        self, agent: str, obs: Dict[str, Any], action: Dict[str, Any]
    ):
    ...
    def action_masks(self, agent: str, obs: Dict[str, Any]):
        ...
        if len(units.keys()) != 0:
            action_mask = np.zeros((self.total_act_dims,len(units.keys())), dtype=bool)

        for unit_idx,unit_id in enumerate(units.keys()):

            unit_action_mask = np.zeros(self.total_act_dims)
            ...
            action_mask[:,unit_idx] = unit_action_mask
